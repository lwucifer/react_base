...
// import file
import { FetchNumberReceptions, setTitlePage} from '../../actions/index'
...
class NumberReceptions extends Component {
    constructor(props) {
		super(props);
		this.state = {
    ...
    componentWillMount(){
        this.props.setTitle('受信回数');
        if(isEmpty( this.props.number_receptions )){
            this.getNumberReceptions();
        }
    }
    ...
    render() {
        const { t } = window.lang;
        let { number_receptions } = this.props;
        return (
            <div className="row">
                ...
                <div className="col-sm-10">
                    <React.Suspense fallback={renderLoader()}>
                        ...
                        >
                            {number_receptions && number_receptions.length > 0 && number_receptions.map( (item, key) => {
                                return (
                                    <tr key = {key} >
    ...
    getNumberReceptions = (meta = null) => {
        ...
        if(moment(params.start_time).diff(params.end_time, 'minutes') > 1){
            ...
            this.props.getNumberReceptions(data);
        }
    }
    ...
    handleSearch = () => {
        ...
        this.getNumberReceptions(params);
    }
...
const mapStateToProps = (state) => {
    ...
    if(state.ranking.number_receptions && state.ranking.number_receptions.pagination && state.ranking.number_receptions.pagination.total){
        total = state.ranking.number_receptions.pagination.total;
        page = state.ranking.number_receptions.pagination.current_page;
    }
	return {
        number_receptions: state.ranking.number_receptions ? state.ranking.number_receptions.list : [],
        totalRecord: total,
        page: page
	};
}
const mapDispatchToProps = (dispatch, props) => {
	return {
		...
		getNumberReceptions : (params) => {
            dispatch(FetchNumberReceptions(params));
        }
	}
}
export default connect(mapStateToProps, mapDispatchToProps)(NumberReceptions);