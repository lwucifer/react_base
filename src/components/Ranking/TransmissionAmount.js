...
// import file
import { FetchTransmissionAmount, setTitlePage} from '../../actions/index';
...
class TransmissionAmount extends Component {
    constructor(props) {
		super(props);
		this.state = {
    ...
    componentWillMount(){
        this.props.setTitle('送信量');
        if(isEmpty( this.props.transmissions )){
            this.getTransmissionAmount();
        }
    }
            ...
            <div className="row">
                ...
                <div className="col-sm-10">
                    <React.Suspense fallback={renderLoader()}>
                        ...
                        >
                            {transmissions && transmissions.length > 0  && transmissions.map( (item, key) => {
                                return (
                                    <tr key = {key} >
    ...
    getTransmissionAmount = (meta = null) => {
        let params = {
        ...
        if(moment(params.start_time).diff(params.end_time, 'minutes') > 1){
            ...
            this.props.getTransmissionAmount(data);
        }
    }
    ...
    handleSearch = () => {
        ...
        this.getTransmissionAmount(params);
    }
...
const mapStateToProps = (state) => {
	...
	return {
        transmissions: state.ranking.transmissions ? state.ranking.transmissions.list : [],
        totalRecord: total,
        page: page
	};
}
const mapDispatchToProps = (dispatch, props) => {
	return {
		...
		getTransmissionAmount : (params) => {
            dispatch(FetchTransmissionAmount(params));
        }
	}
}
export default connect(mapStateToProps, mapDispatchToProps)(TransmissionAmount);