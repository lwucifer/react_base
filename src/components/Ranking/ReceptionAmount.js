...
// import file
import { FetchReceptionsAmount, setTitlePage} from '../../actions/index'
...
class ReceptionAmount extends Component {
    constructor(props) {
		super(props);
		this.state = {
    ...
    componentWillMount(){
        this.props.setTitle('受信量');
        if(isEmpty( this.props.receptions )){
            this.getReceptionsAmount();
        }
    }
            ...
            <div className="row">
                    ...
                    <table className="table table-bordered">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td className="title-second second-datetime">
                ...
                <div className="col-sm-10">    
                    <React.Suspense fallback={renderLoader()}>
                        ...
                        >
                            {receptions && receptions.length > 0 && receptions.map( (item, key) => {
                                return (
                                    <tr key = {key} >
    ...
    getReceptionsAmount = (meta = null) => {
        ...
        if(moment(params.start_time).diff(params.end_time, 'minutes') > 1){
            ...
            if(localStorage.getItem(Config.ACCESS_TOKEN)){
                this.props.getReceptionsAmount(data);
            }
        }
    }
    ...
    handleSearch = () => {
        ...
        this.getReceptionsAmount(params);
    }
...
const mapStateToProps = (state) => {
    ...
    if(state.ranking.receptions && state.ranking.receptions.pagination && state.ranking.receptions.pagination.total){
        total = state.ranking.receptions.pagination.total;
        page = state.ranking.receptions.pagination.current_page;
    }
	return {
        receptions: state.ranking.receptions ? state.ranking.receptions.list : [],
        totalRecord: total,
        page: page
	};
}
const mapDispatchToProps = (dispatch, props) => {
	return {
		...
		getReceptionsAmount : (params) => {
            dispatch(FetchReceptionsAmount(params));
        }
	}
}
export default connect(mapStateToProps, mapDispatchToProps)(ReceptionAmount);